...
class RnnModel(nn.Module):
    ...
    def forward_lm(self, inpt_emb, lang_h, ctx_h):
        ...
        inpt_emb = inpt_emb.unsqueeze(2)
       
        inpt_emb = inpt_emb.expand_as(ctx_h_rep)
        test0= ctx_h_rep.size(0)
        test1= ctx_h_rep.size(1)
        zeros_tensor = torch.zeros(test0, test1, 192)
        inpt_emb = torch.cat((inpt_emb, zeros_tensor), dim=2)
    ...
    def forward_selection(self, inpt_emb, lang_h, ctx_h):
        ...
        inpt_emb = inpt_emb.unsqueeze(2)
        inpt_emb = inpt_emb.expand_as(lang_h)
        test0= lang_h.size(0)
        test1= lang_h.size(1)
        zeros_tensor = torch.zeros(test0, test1, 128)
        inpt_emb = torch.cat((inpt_emb, zeros_tensor), dim=2)

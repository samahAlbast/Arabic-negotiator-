...
class DialogLogger(object):
    ...
    def __init__(self, verbose=False, log_file=None, append=False):
        ...
        if log_file:
            ...
            self.logs.append(open(log_file, flags,encoding='utf-8'))

    def _dump(self, s, forced=False):
        reshaped_s = arabic_reshaper.reshape(s)
        displayed_s = get_display(reshaped_s)
        for log in self.logs:
            print(displayed_s, file=log)
            log.flush()
        if forced:
            print(displayed_s, file=sys.stdout)
            sys.stdout.flush()
    ...
    def dump_sent(self, name, sent):
        self._dump_with_name(name, get_display(arabic_reshaper.reshape(' '.join(sent))).replace('<eos>',''))
